...
def benchmark_thresholds():
    ...
    """
    print(
        f"{'Size (Elem)':<12} {'Bytes':<12} {'CPU (ms)':<10} {'GPU (ms)':<10} {'Winner':<8}"
    )
    print("-" * 60)
    ...
    for n in sizes:
        ...
        if n == 100_000:
            if winner == "GPU" or abs(cpu_t - gpu_t) < 0.01:
                print(
                    "  -> Validation: 100k is a reasonable transition point for these params."
                )
            else:
                print(
                    "  -> Validation: Warning, 100k might be sub-optimal based on this synthetic model."
                )
